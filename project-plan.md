...
USER STORY: As a user, I want to be able to search for banks, so I can find the data I need | Able to get a list of all banks partially matching an input via a UI or via a REST API call. API call returns JSON array|
USER STORY: As a user, I want to be able to look at the edit history of a bank and revert it, so I can fix people's errors | Bank nodes are versioned.
